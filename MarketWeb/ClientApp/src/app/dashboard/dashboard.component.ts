...
export class DashboardComponent {
  ...
  constructor(
    ...
    this.dashboardService.GetNames().subscribe(
      (result) => {
    ...
    timer(0, reloadInterval)
      .pipe(mergeMap(async () => this.updateTable()))
      .subscribe();
  }

  updateTable() {
    this.dashboardService.GetCompanyPrices().subscribe(
      (result) => {
  ...
  changeCompanyValue() {
    if (this.addOrUpdatePriceForm.get("CompanyId").value) {
      this.dashboardService
        .GetPricesByCompanyId(
          Number(this.addOrUpdatePriceForm.get("CompanyId").value)
        )
        .subscribe(
          (result) => {
  ...
  showCompanyCurrentPrice() {
    var existCompanyPrice = this.companyMarketPrice.find(
      (x) =>
        x.companyId ===
          Number(this.addOrUpdatePriceForm.get("CompanyId").value) &&
    ...
    this.companyPrice =
      existCompanyPrice != null ? existCompanyPrice.price : null;
  }
  ...
  onSubmit() {
    ...
    if (this.addOrUpdatePriceForm.valid) {
      this.dashboardService
        .AddOrUpdateCompanyPrice(this.addOrUpdatePriceForm.value)
        .subscribe(
          (result) => {
